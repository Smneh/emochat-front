...
export class VisitTimePipe implements PipeTransform {
  transform(value: string): string {
    ...
    if (
      ...
      return `today ${this.formatTime(lastActionTime)}`;
    } else {
      ...
      if (
        ...
        return `yesterday ${this.formatTime(lastActionTime)}`;
      } else {
        // For dates before today and yesterday
        return `${this.formatDate(lastActionTime)} at ${this.formatTime(lastActionTime)}`;
      }
    }
  }
